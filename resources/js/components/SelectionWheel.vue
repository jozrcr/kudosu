<template>
    ...
    <div class="absolute z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        <div class="w-32 lg:w-40 h-32 lg:h-40 rounded-full  custom-circle relative flex items-center justify-center" ref="wheel">
            <div v-for="number in numbers" :key="number" class="number-button absolute w-12 h-12 bg-white border border-gray-100 rounded-full cursor-pointer hover:bg-gray-200 ease-in-out duration-100 transition-all text-black flex items-center justify-center drop-shadow-sm shadow-md" @click="selectNumber(number)">
                {{ number }}
            </div>
        </div>
    </div>
