<template>
  <div
    class="fixed lg:absolute z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/50 w-screen lg:w-fit lg:h-fit h-screen p-1 lg:rounded-3xl backdrop-blur-sm flex flex-col items-center justify-center"
    @click="hideWheel()"
  >
    <button
    ...
    <button
      class="select-none  w-8 h-8 md:w-10 md:h-10 text-sm sm:text-base lg:hidden  bg-white border border-gray-100 rounded-full cursor-pointer hover:bg-gray-200 ease-in-out duration-100 transition-all text-black flex items-center justify-center drop-shadow-sm shadow-md"
...
const emit = defineEmits(['select', 'hide', 'empty'])
